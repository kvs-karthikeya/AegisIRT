...
export async function POST(request: NextRequest) {
  try {
    ...
    if (action === "heal_element") {
      ...
      let parsedAiChoice;
      let finalLocator = { type: "", value: "", confidence: 0.9 };
  ...
  "reasoning": "string (brief explanation)"
      ...
      while (attempt < 3 && !verificationResult) {
        ...
        try {
          ...
          let safeConf = parseFloat(parsedAiChoice.confidence);
          if (isNaN(safeConf)) safeConf = 0.92;

          finalLocator = { type: parsedAiChoice.type, value: parsedAiChoice.value, confidence: safeConf };
